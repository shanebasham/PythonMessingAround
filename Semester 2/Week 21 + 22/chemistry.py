...
def main():
    ...
    mass = input('Enter mass of chemical in grams: ')
    ...
    compound_list = parse_formula(formula, periodic_table_dic)

    # Call the compute_molar_mass function to compute the molar mass of the molecule from the compound list.
    molar_mass = compute_molar_mass(compound_list, periodic_table_dic)
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    quantity = str(symbol_quantity_list[1])
    # q = ''.join(map(str, quantity))
        # Get the atomic mass for the symbol from the dictionary.
    atomic_mass = str(symbol[ATOMIC_MASS_INDEX])
    # a_mass = int(''.join(map(str, atomic_mass)))
        # Multiply the atomic mass by the quantity.
    product = a_mass * q
        # Add the product into the total molar mass.
    molar_mass = sum(product)
